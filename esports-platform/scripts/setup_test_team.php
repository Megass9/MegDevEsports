...
if (!$team) {
    $team = \App\Models\Team::create([
        'name' => 'Admin Test Takimi',
        'captain_id' => $user->id,
        'game' => 'valorant',
        'description' => 'Test icin otomatik olusturuldu'
    ]);
...
if (!$tournament) {
    $tournament = \App\Models\Tournament::where('status', 'registration')->first();
}
...
if (!$tournament->participants()->where('team_id', $team->id)->exists()) {
    ...
    echo 'Team registered to tournament.' . "\n";
} else {
...
if ($tournament->status === 'in_progress') {
    ...
    foreach ($matches as $m) {
        echo '  Match #' . $m->id . ': ' . ($m->team1->name ?? 'TBD') . ' vs ' . ($m->team2->name ?? 'TBD') . ' (Status: ' . $m->status . ')' . "\n";
    }
}
